...
def jaccard(_box_a, _box_b):
    # 计算真实框的左上角和右下角
    # 0的位置全是0,2的位置是宽的长度,全是0意味着假设中心都是一样的进行计算，全是1也是可以的
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        # ----------------------------------------------------------------#
        # 这一步只是单纯地处理label里的gt和该特征图下的anchor的关系，跟input的数据还没有关系
        # 得到gt的位置信息，还有对应的anchor的置信度和种类信息
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        ...
        for b in range(bs):
            ...
            #   9, 4
            # -------------------------------------------------------#
            # 这里的anchors是缩放到特征图上的宽高，如果忘了就到上面看一看
            ...
            for i, best_n in enumerate(best_ns):
                # 如果跟gt最大iou的框不是该特征图上的框，怎么搞？
                if best_n not in anchor_index:
                    continue
                # -------------------------------------------------------------#
                #   取出各类坐标：
                #   gi和gj代表的是真实框对应的特征点的x轴y轴坐标
                #   gx和gy代表真实框中心点的x轴和y轴坐标
                #   gw和gh代表真实框的宽和高
                # -------------------------------------------------------------#
                gi = gis[i].long()
                gj = gjs[i].long()
                ...
                if (gj < in_h) and (gi < in_w):
                    # best是9个中的索引，减去起始索引，表示是该特征图三个anchor中第几个位置的anchor
